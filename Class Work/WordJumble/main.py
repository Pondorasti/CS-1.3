...
def solve_one_jumble(letters):
    ...
    def permutations(characters, ans = ""):
      if characters == "":
        if english_dict.check(ans):
          valid_words.add(ans)
        return
      
      for char in characters: 
        permutations(characters.replace(char, "", 1), ans + char)
...
def solve_final_jumble(letters, final_circles):
    ...
    Parameters:
    - letters: string, the scrambled letters for a single word
    - final_circles: list of strings with O (letter "oh") that shows
    ...
    def phrasePermutations(characters, index = 0, solution = [""]):
      # print(solution)
      if len(solution[index]) == group_sizes[index]:
        if english_dict.check(solution[index]): # check if current word exists
          if len(solution) == len(group_sizes): # return solution if size is met
            valid_phrases.add(tuple(solution))
          else:
            solution.append("")
            phrasePermutations(characters, index + 1, solution)
        
        return
      elif len(solution[index]) > group_sizes[index]:
        return
      
      for char in characters:
        new_word = solution[index] + char
        new_solution = copy.deepcopy(solution)
        new_solution[index] = new_word
        phrasePermutations(characters.replace(char, "", 1), index, new_solution)

    phrasePermutations(letters)
